...
import { getAllUsers, type User, type PaginatedUsersResponse } from '@/lib/modules/account';
...
export default function UsuariosPage() {
  ...
  const fetchUsers = async (page: number = 1) => {
    try {
      setLoading(true);
      const response: PaginatedUsersResponse = await getAllUsers(page, USUARIOS_CONFIG.limit);
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-3xl font-bold text-gray-800">{USUARIOS_CONFIG.title}</h1>
